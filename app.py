...
try:
    ...
    # [수정] 헌터 예수금 밑에 블리츠 예수금 배치 (컬럼 없이 수직 나열)
    ...
    if menu == "🚀 SOXL 대시보드":
        ...
        # [수정] 매수 신호 분석: 박스 디자인 복구 (구분선 hr 포함)
        st.markdown("---")
        ...
        # [복구] 블리츠 신호 표시
        if is_blitz:
            st.success(f"⚡ **Blitz 신호 발생!** (RSI2 < 5 & 상승장) → 단타 진입 추천 (${wallet['blitz_cash']:,.0f} 사용 가능)")
